...
Configuration is supplied via environment variables, either in a `.env` file (Docker Compose) or as Kubernetes secrets and ConfigMaps (Helm). Sensitive values must never be committed to version control.
...
Store the key as a Kubernetes secret or Docker secret — do not place it in the `.env` file in production.
